...
function generateBenchmarkFile({
    ...
    functions = ['makeoffer'],
    contractId = process.env.CHAINCODE_NAME || process.env.CC_NAME || 'coffee_9',
    channel = 'coffee1003',
    txNumber = 5000,
    startRate = 50,
    rateStep = 50,
    ...
    testCoffeeLotQueryLimit = process.env.TEST_CARDAMOM_LOT_QUERY_LIMIT || 20,
    testCoffeeMaxLotQueryLimit = process.env.TEST_CARDAMOM_MAX_LOT_QUERY_LIMIT || 3000,
    testCoffeeLotQueryBuffer = process.env.TEST_CARDAMOM_LOT_QUERY_BUFFER || 1.5,
    testCoffeeSetupLotQueryDivisor = process.env.TEST_CARDAMOM_SETUP_LOT_QUERY_DIVISOR || setupLotQueryDivisor,
    ...
    const testCoffeeMatch = functionName.match(/testcoffee_a(\d+)(?:_s(\d+))?$/i);
    ...
    if (testCoffeeMatch) {
        argumentLines.push(`          aggregatorCount: ${testCoffeeMatch[1]}`);
        if (testCoffeeMatch[2]) {
            argumentLines.push(`          aggregatorStartIndex: ${testCoffeeMatch[2]}`);
        }
    }
    ...
    function getRoundArgumentLines(currentTps) {
        ...
        if (testCoffeeMatch) {
            const aggregatorCount = Math.max(
                1,
                testCoffeeMatch[2]
                    ? Number(testCoffeeSetupLotQueryDivisor)
                    : Number(testCoffeeMatch[1]) || 1
            );
            const minLimit = Math.max(1, Number(testCoffeeLotQueryLimit) || 1);
            const maxLimit = Math.max(minLimit, Number(testCoffeeMaxLotQueryLimit) || 3000);
            const buffer = Math.max(1, Number(testCoffeeLotQueryBuffer) || 1);
...
`;
    ...
    const yamlContent = `
test:
  name: Coffee Chaincode Load Test
