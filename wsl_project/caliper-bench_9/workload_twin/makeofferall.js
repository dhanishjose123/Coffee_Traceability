...
class MakeOfferAllWorkload extends WorkloadModuleBase {
    ...
    async initializeWorkloadModule(workerIndex, totalWorkers, roundIndex, roundArguments, sutAdapter) {
        ...
        /* ---------------------------------
           ðŸ”¹ Fetch APPROVED lots WITHOUT offers
        ---------------------------------- */
        const lotQuery = {
        ...
        try {
            ...
            if (!resultString) {
                console.warn(`âš ï¸ Worker ${this.workerIndex}: Empty response`);
                return;
            }

            const allLots = JSON.parse(resultString);

            // âœ… Direct assignment (already filtered)
            this.availableLots = [...allLots];

            console.log(
                `[INIT][Worker ${this.workerIndex}] ðŸ“¦ Loaded ${this.availableLots.length} lots for ${this.farmerUserId || 'all farmers'}`
            );
