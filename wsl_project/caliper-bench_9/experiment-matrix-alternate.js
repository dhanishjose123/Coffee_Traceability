...
const channelName = 'agrochannel1707';
...
// Setup TPS values also define the number of setup rounds.
const setupTpsLoads = [500];

// Per-round durations used by run-alternate.js.
const txDurationSeconds = 5;
const setupTxDurationSeconds = 10;
const functionTxDurations = {
    submitNoMvcc: 31,
    testCoffeeSetup: 25,
    testCoffeeAlternate: 19,
    makeOfferAllSetup: 15,
    acceptNoMvcc: 12,
    packSetup: 9,
    purchase: 6
};
...
function testCoffeeBenchmarks() {
    return strategicCounts.map(count => `testcoffee_a${count}`);
}

function testCoffeeBenchmarks1() {
    return strategicCounts1.map(count => `testcoffee_a${count}`);
}

function testCoffeeSetup(participantCount = 24) {
    const total = Number(participantCount) || setupParticipantCount;
    return chunkParticipantCount(total).map(({ count, start }) =>
        `testcoffee_a${count}_s${start}`
    );
}
...
function allBenchmarks() {
    return uniqueList([
        ...
        ...testCoffeeBenchmarks(),
        ...testCoffeeSetup(setupParticipantCount),
        ...makeOfferBenchmarks(),
...
function benchmarkGroups() {
    const groups = {
        ...
        'testcoffee-setup': submitProduceNoMvccSetup(setupParticipantCount),
        testcoffee: testCoffeeBenchmarks(),
        'testcoffee-alternate': testCoffeeSetup(setupParticipantCount),
