...
function getGroupBenchmarks(groupName) {
    ...
    if (key === 'testcoffee' || key === 'test') {
        return matrix.testCoffeeBenchmarks();
    }

    if (key === 'testcoffee-setup' || key === 'test-setup') {
        return matrix.submitProduceNoMvccSetup(matrix.setupParticipantCount);
    }

    if (key === 'testcoffee-alternate' || key === 'test-alternate' || key === 'testcoffee-batches') {
        return matrix.testCoffeeSetup(matrix.setupParticipantCount);
    }
...
process.env.SETUP_TRANSACTION_LOADS = process.env.SETUP_TRANSACTION_LOADS ||
    process.env.LATENCY_SETUP_TPS ||
    matrix.setupTpsLoads.join(',');
process.env.TX_DURATION_SECONDS = process.env.TX_DURATION_SECONDS ||
    String(matrix.txDurationSeconds);
process.env.SETUP_TX_DURATION_SECONDS = process.env.SETUP_TX_DURATION_SECONDS ||
    String(matrix.setupTxDurationSeconds);
process.env.SUBMIT_NO_MVCC_TX_DURATION_SECONDS = process.env.SUBMIT_NO_MVCC_TX_DURATION_SECONDS ||
    String(matrix.functionTxDurations.submitNoMvcc);
process.env.TESTCOFFEE_SETUP_TX_DURATION_SECONDS = process.env.TESTCOFFEE_SETUP_TX_DURATION_SECONDS ||
    String(matrix.functionTxDurations.testCoffeeSetup);
process.env.TESTCOFFEE_ALTERNATE_TX_DURATION_SECONDS = process.env.TESTCOFFEE_ALTERNATE_TX_DURATION_SECONDS ||
    String(matrix.functionTxDurations.testCoffeeAlternate);
process.env.MAKEOFFERALL_SETUP_TX_DURATION_SECONDS = process.env.MAKEOFFERALL_SETUP_TX_DURATION_SECONDS ||
    String(matrix.functionTxDurations.makeOfferAllSetup);
process.env.ACCEPT_NO_MVCC_TX_DURATION_SECONDS = process.env.ACCEPT_NO_MVCC_TX_DURATION_SECONDS ||
    String(matrix.functionTxDurations.acceptNoMvcc);
process.env.PACK_SETUP_TX_DURATION_SECONDS = process.env.PACK_SETUP_TX_DURATION_SECONDS ||
    String(matrix.functionTxDurations.packSetup);
process.env.PURCHASE_TX_DURATION_SECONDS = process.env.PURCHASE_TX_DURATION_SECONDS ||
    String(matrix.functionTxDurations.purchase);
...
console.log(`Setup TPS      : ${process.env.SETUP_TRANSACTION_LOADS}`);
console.log(`Durations      : tx=${process.env.TX_DURATION_SECONDS}s, setup=${process.env.SETUP_TX_DURATION_SECONDS}s`);
