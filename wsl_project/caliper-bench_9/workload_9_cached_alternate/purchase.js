...
class PurchasePacketWorkload extends WorkloadModuleBase {
    ...
    async initializeWorkloadModule(workerIndex, totalWorkers, roundIndex, roundArguments, sutAdapter) {
        ...
        this.assignedConsumerIds = getAssignedUserIds(1, this.consumerCount, workerIndex, totalWorkers);
        this.userId = this.assignedConsumerIds[0] || `User${consumerIndex}`;
        this.resetClaimDirAtBenchmarkStart();

        console.log(
            `✅ Worker ${this.workerIndex}: Initialized consumer ${this.userId}, assignedConsumers=${this.assignedConsumerIds.join('|')}, consumerCount=${this.consumerCount}, retailerOwnerRange=${this.retailerCount > 0 ? `User1..User${this.retailerCount}` : 'all'}, mode=${this.retailerSelectionMode}`
        );
        ...
        const cacheKey = [
            ...
            this.assignedConsumerIds.join('-') || this.userId,
        ...
        try {
            ...
            for (const consumerId of this.assignedConsumerIds) {
                const queryTx = this.createAvailablePacketQuery(consumerId);
                const res = await this.sutAdapter.sendRequests(queryTx);

                if (!res?.status?.result) {
                    console.log(`[INIT][Worker ${this.workerIndex}] ❌ No packets returned for ${consumerId}`);
                    continue;
                }

                const allPackets = JSON.parse(res.status.result.toString());
                const packets = (Array.isArray(allPackets) ? allPackets : (allPackets.data || []))
                    .filter(p => p.status === 'AVAILABLE')
                    .filter(p => this.isAllowedRetailerOwner(p.owner, consumerId));
                packets.forEach(packet => {
                    packet.__consumerUserId = consumerId;
                    loadedPackets.push(packet);
                });
            }

            this.availablePackets = this.partitionPackets(loadedPackets);
            this.packetsLoaded = this.availablePackets.length;
            if (preloadCache.isWriteMode()) {
                preloadCache.write(cacheKey, this.availablePackets);
            }

            console.log(`[INIT][Worker ${this.workerIndex}] 📦 Cached ${this.availablePackets.length} AVAILABLE packets for ${this.assignedConsumerIds.join('|')}`);
    ...
    async submitTransaction() {
        ...
        const buyerId = packet.__consumerUserId || this.userId;
    ...
    partitionPackets(packets) {
        return packets.filter((packet, index) =>
            ...
            this.isAllowedRetailerOwner(packet.owner, packet.__consumerUserId) &&
    ...
    isAllowedRetailerOwner(ownerId, consumerId = this.userId) {
        if (this.retailerSelectionMode === 'paired') {
            return ownerId === consumerId;
        }
    ...
    createAvailablePacketQuery(consumerId = this.userId) {
        ...
        if (this.retailerSelectionMode === 'paired') {
            return {
                ...
                contractArguments: [consumerId, 'AVAILABLE', queryLimit],
                readOnly: true
            };
        }
