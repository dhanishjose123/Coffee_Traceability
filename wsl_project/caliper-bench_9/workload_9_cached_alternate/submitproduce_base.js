...
class SubmitProduceWorkload extends WorkloadModuleBase {
    ...
    constructor(aggregatorCount = 5, farmerCount = 5) {
        ...
        this.farmerSelectionMode = 'worker';
    ...
    async initializeWorkloadModule(workerIndex, totalWorkers, roundIndex, roundArguments, sutAdapter) {
        ...
        this.farmerSelectionMode = String(roundArguments.farmerSelectionMode || process.env.SUBMITPRODUCE_FARMER_SELECTION_MODE || 'worker').toLowerCase();
        ...
        this.userId = this.getWorkerFarmerId();
        this.invokerIdentity = this.userId;

        const aggregatorEndIndex = this.aggregatorStartIndex + this.aggregatorCount - 1;
        console.log(`Worker ${workerIndex} initialized as ${this.userId}, farmerMode=${this.farmerSelectionMode}, farmerRange=User${this.farmerStartIndex}..User${this.farmerStartIndex + this.farmerCount - 1}, aggregatorMode=${this.aggregatorSelectionMode}, aggregators=User${this.aggregatorStartIndex}..User${aggregatorEndIndex}`);
    }
    ...
    getFarmerId() {
        if (this.farmerSelectionMode === 'random') {
            const farmerIndex = crypto.randomInt(
                this.farmerStartIndex,
                this.farmerStartIndex + this.farmerCount
            );
            return `User${farmerIndex}`;
        }

        return this.getWorkerFarmerId();
    }
...
Aggregator Range: User${this.aggregatorStartIndex}..User${this.aggregatorStartIndex + this.aggregatorCount - 1}
	Selection Mode: ${this.farmerSelectionMode} farmer, ${this.aggregatorSelectionMode} aggregator per transaction
