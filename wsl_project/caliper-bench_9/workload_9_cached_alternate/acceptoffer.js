...
class AcceptOfferWorkload extends WorkloadModuleBase {
    ...
    this.assignedFarmerIds = getAssignedUserIds(this.farmerStartIndex, this.farmerCount, workerIndex, totalWorkers);
    this.farmerUserId = this.assignedFarmerIds[0] || `User${farmerUserIndex}`;

    // 🔥 ALWAYS initialize
    this.availableLots = [];

    console.log(`Worker ${this.workerIndex}: Initialized farmer ${this.farmerUserId}, assignedFarmers=${this.assignedFarmerIds.join('|')}, farmerRange=User${this.farmerStartIndex}..User${this.farmerStartIndex + this.farmerCount - 1}, retailerMode=${this.retailerSelectionMode}, retailerRange=User${this.retailerStartIndex}..User${this.retailerStartIndex + this.retailerCount - 1}`);
    ...
    const cacheKey = [
        ...
        this.assignedFarmerIds.join('-') || this.farmerUserId,
    ...
    try {
        ...
        for (const farmerId of this.assignedFarmerIds) {
            const queryTx = {
                contractId: this.contractId,
                contractFunction: 'getLotsWithOffersByOwner',
                contractArguments: [farmerId, String(this.queryLimit)],
                readOnly: true
            };
            const res = await this.sutAdapter.sendRequests(queryTx);
            if (!res?.status?.result) {
                console.log(`[INIT][Worker ${this.workerIndex}] ❌ No lots for ${farmerId}`);
                continue;
            }

            const resultBuffer = res.status.result;
            console.log(`[Worker ${this.workerIndex}] 📏 Payload bytes for ${farmerId}: ${resultBuffer.length}`);
            let parsed;
            try {
                parsed = JSON.parse(resultBuffer.toString());
            } catch (err) {
                console.error(`[INIT][Worker ${this.workerIndex}] ❌ JSON parse failed for ${farmerId}`);
                continue;
            }

            const lots = Array.isArray(parsed)
                ? parsed
                : parsed.data && Array.isArray(parsed.data)
                    ...
                    : typeof parsed === 'object' && parsed !== null
                        ...
                        : [];
            lots.forEach(lot => {
                lot.__farmerUserId = farmerId;
                loadedLots.push(lot);
            });
        }

        this.availableLots = loadedLots.filter(lot => this.isAllowedRetailerForFarmer(lot?.highestOffer?.retailerId, lot.__farmerUserId));
    ...
    isAllowedRetailer(retailerId) {
        return this.isAllowedRetailerForFarmer(retailerId, this.farmerUserId);
    }

    isAllowedRetailerForFarmer(retailerId, farmerUserId) {
        ...
        if (this.retailerSelectionMode === 'paired' || this.retailerSelectionMode === 'matching') {
            const farmerMatch = String(farmerUserId || '').match(/^User(\d+)$/i);
