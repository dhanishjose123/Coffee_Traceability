...
function getAssignedUserIds(startIndex, count, workerIndex, totalWorkers) {
    const users = [];
    const workers = Math.max(1, Number(totalWorkers) || 1);
    const total = Math.max(1, Number(count) || 1);

    if (total <= 4) {
        return [`User${startIndex + (workerIndex % total)}`];
    }

    for (let offset = 0; offset < total; offset++) {
        if ((offset % workers) === workerIndex) {
            users.push(`User${startIndex + offset}`);
        }
    }
...
/* ============================================================
   🔹 WORKLOAD CLASS
============================================================ */
class TestCoffeeWorkload extends WorkloadModuleBase {
    ...
    async initializeWorkloadModule(workerIndex, totalWorkers, roundIndex, roundArguments, sutAdapter) {
        ...
        this.lotQueryLimit = Math.max(1, Number(roundArguments.lotQueryLimit || process.env.TEST_CARDAMOM_LOT_QUERY_LIMIT || defaultLimit));
        this.assignedAggregatorIds = getAssignedUserIds(
        ...
        this.aggregatorUserId = this.assignedAggregatorIds[0] || `User${aggregatorUserIndex}`;
        this.aggregatorId = this.aggregatorUserId;
        this.invokerIdentity = this.aggregatorUserId;
	
        console.log(`Worker ${workerIndex + 1} initialized as ${this.aggregatorUserId}, assignedAggregators=${this.assignedAggregatorIds.join('|')}, aggregatorRange=User${this.aggregatorStartIndex}..User${this.aggregatorStartIndex + this.aggregatorCount - 1}`);
        ...
        const staggerDelayMs = await staggerInitialLoad(workerIndex, 'testcoffee');
        const queryStartedAt = Date.now();
        const cacheKey = [
            'testcoffee',
            this.contractId,
            roundArguments.channel || process.env.CHANNEL_NAME || 'channel',
            this.assignedAggregatorIds.join('-') || this.aggregatorUserId,
            this.lotQueryLimit
        ];

        if (preloadCache.isReadMode()) {
            this.allLots = preloadCache.read(cacheKey) || [];
            console.log(`[LOAD_TIME][testcoffee][Worker ${this.workerIndex}] source=preload-cache staggerMs=${staggerDelayMs} totalMs=${Date.now() - loadStartedAt} lots=${this.allLots.length}`);
            return;
        }
        ...
        try {
            ...
            for (const aggregatorId of this.assignedAggregatorIds) {
                const lotQuery = {
                    contractId: this.contractId,
                    contractFunction: 'getSubmittedProduceByAggregator',
                    invokerIdentity: aggregatorId,
                    contractArguments: [aggregatorId, String(this.lotQueryLimit)],
                    readOnly: true
                };
                const response = await this.sutAdapter.sendRequests(lotQuery);
                const buffer = response?.status?.result ||
                    (Array.isArray(response) && response[0]?.status?.result) ||
                    (Array.isArray(response) && response[0]);

                if (!buffer) {
                    console.error(`⚠️ Worker ${this.workerIndex}: No buffer result for ${aggregatorId}`);
                    continue;
                }

                const parsed = JSON.parse(buffer.toString());
                const lots = Array.isArray(parsed.data) ? parsed.data : [];
                lots.forEach(lot => {
                    lot.__aggregatorUserId = aggregatorId;
                    loadedLots.push(lot);
                });
            }

            this.allLots = loadedLots;
            if (preloadCache.isWriteMode()) {
                preloadCache.write(cacheKey, this.allLots);
            }

            console.log(`📦 Worker ${this.workerIndex}: Cached ${this.allLots.length} lots from ${this.assignedAggregatorIds.join('|')}`);
            console.log(`[LOAD_TIME][testcoffee][Worker ${this.workerIndex}] staggerMs=${staggerDelayMs} queryMs=${Date.now() - queryStartedAt} totalMs=${Date.now() - loadStartedAt} lots=${this.allLots.length}`);
    ...
    async submitTransaction() {
        ...
        const testTx = {
            contractId: this.contractId,
            contractFunction: 'testCoffee',
            invokerIdentity: lot.__aggregatorUserId || this.invokerIdentity,
        ...
        if (this.txIndex < 50) {
            fs.appendFileSync(
                this.payloadFile,
                `testCoffee,${payloadBytes},${payloadKB.toFixed(4)}\n`
            );
        }
