...
class PackLotWorkload extends WorkloadModuleBase {
    ...
    async initializeWorkloadModule(workerIndex, totalWorkers, roundIndex, roundArguments, sutAdapter) {
        ...
        this.assignedRetailerIds = getAssignedUserIds(this.retailerStartIndex, this.retailerCount, workerIndex, totalWorkers);
        this.retailerUserId = this.assignedRetailerIds[0] || `User${retailerIndex}`;

        console.log(`✅ Worker ${this.workerIndex}: Retailer ${this.retailerUserId}, assignedRetailers=${this.assignedRetailerIds.join('|')}`);
        ...
        const cacheKey = [
            ...
            this.assignedRetailerIds.join('-') || this.retailerUserId,
            this.lotQueryLimit
        ];
        ...
        try {
            ...
            for (const retailerId of this.assignedRetailerIds) {
                const lotQuery = {
                    contractId: this.contractId,
                    contractFunction: 'getProduceByStatusAndOwner',
                    contractArguments: [`SOLD`, retailerId, String(this.lotQueryLimit)],
                    readOnly: true
                };
                ...
                if (!buffer) {
                    console.error(`⚠️ Worker ${this.workerIndex}: No buffer result for ${retailerId}`);
                    continue;
                }

                const resultString = buffer.toString();
                console.log(`📥 Worker ${this.workerIndex} result length for ${retailerId}: ${resultString.length}`);
                const parsed = JSON.parse(resultString);
                const lots = parsed.data || [];
                lots.forEach(lot => {
                    lot.__retailerUserId = retailerId;
                    loadedLots.push(lot);
                });
            }
            ...
            console.log(`📦 Worker ${this.workerIndex}: Cached ${this.allLots.length} lots for ${this.assignedRetailerIds.join('|')}`);
