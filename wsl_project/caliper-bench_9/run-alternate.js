...
const txNumber = 500;
const txDurationSeconds = Number(process.env.TX_DURATION_SECONDS || 5);
...
const contractId = process.env.CHAINCODE_NAME || 'coffee_11';
...
function testCoffeeBenchmarks() {
    return strategicCounts.map(count => ({
        functionName: `testcoffee_a${count}`,
        logLevel: 'info'
    }));
}

function testCoffeeBatches(participantCount = 24, startIndex = Number(process.env.TEST_CARDAMOM_BATCH_START || 1)) {
    const batches = [];
    let start = Math.max(1, Number(startIndex) || 1);
    const total = Math.max(1, Number(participantCount) || 1);
    ...
    while (start <= total) {
        const batchSize = Math.min(5, total - start + 1);
        batches.push({
            functionName: `testcoffee_a${batchSize}_s${start}`,
            logLevel: 'info'
        });
        start += batchSize;
    }
...
const testCoffeeMatrix = testCoffeeBenchmarks();
const testCoffeeAlternateMatrix = testCoffeeBatches(24);
...
function getTxDurationForFunction(functionName) {
    ...
    if (/^testcoffee_a\d+_s\d+$/.test(name)) {
        return Number(process.env.TESTCOFFEE_ALTERNATE_TX_DURATION_SECONDS || process.env.TESTCOFFEE_SETUP_TX_DURATION_SECONDS || 18);
    }
...
function getFabricTimeoutConfig(functionName = '') {
    const isTestCoffee = String(functionName).toLowerCase().startsWith('testcoffee_a');
    const evaluateTimeoutSeconds = isTestCoffee
        ? Number(process.env.TEST_CARDAMOM_FABRIC_EVALUATE_TIMEOUT_SECONDS || 180)
        : fabricEvaluateTimeoutSeconds;
    const requestTimeoutMs = isTestCoffee
        ? Number(process.env.TEST_CARDAMOM_FABRIC_REQUEST_TIMEOUT_MS || evaluateTimeoutSeconds * 1000)
        : fabricRequestTimeoutMs;
    const connectionRequestTimeoutMs = isTestCoffee
        ? Number(process.env.TEST_CARDAMOM_FABRIC_CONNECTION_REQUEST_TIMEOUT_MS || requestTimeoutMs)
...
function getNetworkUserRangeForFunction(functionName) {
    ...
    if (testCoffeeMatch) {
        const start = Number(testCoffeeMatch[2] || 1);
        return { start, end: start + Number(testCoffeeMatch[1]) - 1 };
    }
...
function getRequiredOrgsForFunction(functionName) {
    ...
    if (name.startsWith('testcoffee_a')) {
        return ['aggregators'];
    }
...
function getWorkersForFunction(functionName) {
    ...
    const regularTestCoffeeMatch = String(functionName).match(/^testcoffee_a(\d+)$/i);
    if (regularTestCoffeeMatch) {
        const workers = Number(process.env.ALTERNATE_TESTCOFFEE_WORKERS || process.env.ALTERNATE_LOGICAL_WORKERS || 5);
        return Number.isFinite(workers) && workers > 0 ? workers : Number(regularTestCoffeeMatch[1]);
    }
    ...
    const testCoffeeMatch = String(functionName).match(/^testcoffee_a(\d+)(?:_s(\d+))?$/i);
    if (testCoffeeMatch) {
        const aggregatorCount = Number(testCoffeeMatch[1]);
        if (aggregatorCount <= 4) {
            return getAlternateWorkerCount('ALTERNATE_TESTCOFFEE_WORKERS');
        }
