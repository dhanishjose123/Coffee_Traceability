...
const chaincodeName = process.env.CHAINCODE_NAME || 'coffee_9';
...
function testCoffeeBatches(
    participantCount = 24,
    startIndex = Number(process.env.TEST_CARDAMOM_BATCH_START || setupBatchStart)
) {
    const batches = [];
    let start = Math.max(1, Number(startIndex) || 1);
    const total = Math.max(1, Number(participantCount) || 1);
    ...
    while (start <= total) {
        const batchSize = Math.min(5, total - start + 1);
        batches.push(`testcoffee_a${batchSize}_s${start}`);
        start += batchSize;
    }
...
const defaultBenchmarkGroups = [
    ...
    // },
    // {
    //     name: 'testcoffee-setup',
    //     benchmarks: submitNoMvccSetup(24)
    // },
    // {
    //     name: 'testcoffee-original',
    //     benchmarks: testCoffeeBenchmarks()
    // },
    // {
    //     name: 'testcoffee-alternate',
    //     benchmarks: testCoffeeBatches(24)
    // },
    // {
    //     name: 'makeoffer_1',
...
const setupBatchPatterns = [
    ...
    {
        label: 'TestCoffee setup',
        generated: testCoffeeBatches(24),
        pattern: /^testcoffee_a\d+_s\d+$/i
    },
    {
...
function getSetupTpsForBenchmark(benchmarkName) {
    ...
    if (/^testcoffee_a\d+_s\d+$/.test(name)) {
        return repeatSetupTps(process.env.LATENCY_TESTCOFFEE_SETUP_TPS, '450');
    }
...
function supportsPreloadCache(benchmarkName) {
    ...
    return /^testcoffee_a\d+(?:_s\d+)?$/.test(name) ||
